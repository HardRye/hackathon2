...
const sectionLibrary = document.querySelector(
  'main > .myFilmLibraryPage__container',
);
const btnFavourites = document.querySelector(
...
function activeDetailsPageListener(e) {
  ...
  if (e.currentTarget.dataset.name === 'home') {
    ...
    try {
      ...
      return filmsFromLocalWatched === null
        ? undefined
        : JSON.parse(filmsFromLocalWatched);
    } catch (err) {
      console.error(err);
    }
    ...
    try {
      ...
      return filmsFromLocalQueue === null
        ? undefined
        : JSON.parse(filmsFromLocalQueue);
    } catch (err) {
      console.error(err);
    }
    activeDetailsPage(e.target.dataset.id, true);
...
function activeDetailsPage(movieId, itsLibraryFilm) {
  if (itsLibraryFilm) {
    ...
    if (!selectFilm) {
      selectFilm = filmsFromLocalWatched.find(
